...
public class PropertiesWriter {
	...
	PropertiesWriter(String summary, String description, String dateiname, String projectId, String issueType, String label, String dueDate, String reporter, String assignee)
	{
	...
	{		
		...
		try {
			...
			pWriter.println("\t\"duedate\": \""+ dueDate +"\",");
			if (assignee != null && assignee.trim().length() > 0 && projectId != null && !(projectId.equals("10723"))) {
				pWriter.println("\t\"assignee\":");
				pWriter.println("\t{ \"name\": \""+ assignee + "\" },");
			}
			pWriter.println("\t\"reporter\":");
			pWriter.println("\t{ \"name\": \""+ reporter + "\" }");
			pWriter.println("\t}");
			pWriter.println("}");
	        pWriter.flush();
		} catch (IOException e) {			
